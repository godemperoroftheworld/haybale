package com.t2pellet.tlib.struct;
...
class OctTree implements PosTree {
    ...
    public void insert(final Vec3i pos) {
        if (pos == null) {
            TenzinLib.LOG.error("Tried inserting null BlockPos into OctTree, ignoring");
            return;
        }
        if (!boundary.isInBoundary(pos)) {
            TenzinLib.LOG.error("Attempting to insert point outside of OctTree boundary");
            return;
        }
    ...
    public void delete(final Vec3i pos) {
        if (pos == null) {
            TenzinLib.LOG.error("Tried deleting null pos from OctTree, ignoring");
            return;
        }
        if (!boundary.isInBoundary(pos)) {
            TenzinLib.LOG.error("Attempting to delete point outside of boundary: " + pos);
            return;
        }
        ...
        Object child = children.get(childOctant);
        if (child == null) { // empty
            TenzinLib.LOG.error("Tried deleting pos from null child tree, ignoring");
    ...
    public Vec3i findNearest(Vec3i pos, int maxRange) {
        Queue<Vec3i> pointQueue = rangeSearch(pos, maxRange);
        return pointQueue.poll();
    }

    @Override
    public Queue<Vec3i> rangeSearch(Vec3i pos, int maxRange) {
        if (pos == null) {
            TenzinLib.LOG.error("Tried to find nearest pos to null query - returning null");
            return new LinkedList<>();
        }
        if (!boundary.isInBoundary(pos)) {
            TenzinLib.LOG.error("Attempting to find nearest point outside of boundary: " + pos);
            return new LinkedList<>();
        }
